...
python3 scripts/prove_gate.py            # 工具闸 14/14（含超长输入必交回人审）
...
四个 prove 必须 exit 0，都不需要 Key 或 `typesafe_sdk`。routing／trim_compress 报告落在 `notes/reports/`（可 gitignore）。
...
- 不跟随上游重定向（3xx 原样回给客户端），避免 Key 被带到别的主机。
...
- compress 为**本地启发式 stub**（可证明顺序），不是上游 LLM 压缩；顺序强制 trim → compress。开启后只保留首条 user＋摘要＋最后 3 条；被裁断的 `tool_use`／`tool_result` 会成对剔除。
